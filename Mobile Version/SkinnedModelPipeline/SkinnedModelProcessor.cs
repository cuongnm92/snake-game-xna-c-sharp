...
namespace SkinnedModelPipeline
{
    ...
    public class SkinnedModelProcessor : ModelProcessor
    {
                                             ...
                                             ContentProcessorContext context)
        {
            ...
            //Change The Animation name if the filename contains '@'
            ...
            //2. Process the entire merged data
            ...
            model.Tag = new SkinningData(animationClips, bindPose,
                                         inverseBindPose, skeletonHierarchy, boneNames);

            return model;
        }

        void ChangeAnimationName(NodeContent input, ContentProcessorContext context, string name = null)
        {
            ...
            if (skeleton!=null && skeleton.Animations.Count == 1)
            {
                ...
                if(name.Contains('@'))
                {
                    ...
                    var animationData = skeleton.Animations.Values.ElementAt(0);
                    skeleton.Animations.Clear();

                    skeleton.Animations.Add(name, animationData);
                }
                else
                {
                    context.Logger.LogWarning(null, input.Identity,
                                              "No Custom Animation Name Found, using default name \'{0}\'. \n[Try using myModel@ANIMATION_NAME.fbx]",
                                              skeleton.Animations.Keys.First());
                }
            }
        }
        ...
        {
            var mergeModel =
                context.BuildAndLoadAsset<NodeContent, NodeContent>(new ExternalReference<NodeContent>(mergeFile), null);

            //Change The Animation name if the filename contains '@'
            var fileName = new FileInfo(mergeFile).Name;
            ChangeAnimationName(mergeModel, context, fileName);
            ...
            AnimationContentDictionary animations, IList<BoneContent> bones)
        {
            ...
            {
                AnimationClip processed = ProcessAnimation(animation.Key, animation.Value, boneMap);
        ...
        /// object to our runtime AnimationClip format.
        /// </summary>
        static AnimationClip ProcessAnimation(string animationName, AnimationContent animation,
                                              Dictionary<string, int> boneMap)
        {
            ...
            return new AnimationClip(animationName, animation.Duration, keyframes);
        }
